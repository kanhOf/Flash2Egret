...
namespace annie {
    ...
    export class Shape {
         ...
         * @return {Image}
         */
        public static convertToImage(bitmap:any): any {
            if(bitmap.bitmapData){
                let img=bitmap.bitmapData.source;
                let w: number = bitmap.textureWidth;
                let h: number = bitmap.textureHeight;
                if(w==img.width&&h==img.height){
                    return img;
                }
                let _canvas = Shape._canvas;
                _canvas.width = w;
                _canvas.height = h;
